...
export const CARD_METRICS = {
  ...
  // --- "compact" detail level（schemalens-web 精簡模式專用） ---
  /** 固定寬度，不像其他層級依內容自動計算——精簡模式要對齊成網格。 */
  compactCardWidth: 190,
  /** 表名最多放幾個「半形字元」寬，超過就換行（在底線處優先斷行）。 */
  compactNameWrapChars: 20,
  /** 表名最多幾行，超過就截斷。 */
  compactNameMaxLines: 2,
  compactNameLineHeight: 17,
  /** schema / 欄位數那一行的高度 */
  compactMetaHeight: 16,
  compactPaddingY: 10,
} as const;
...
export interface CardModel {
  ...
  width: number;
  height: number;
  /** 只有 detailLevel === "compact" 才會有值：換行後的表名。 */
  compactNameLines?: string[];
}
...
export function visibleColumns(table: Table, detailLevel: DetailLevel): Column[] {
  if (detailLevel === "overview" || detailLevel === "compact") return [];
  if (detailLevel === "keys") return table.columns.filter(isKeyColumn);
  return [...table.columns];
}

/**
 * 把表名切成固定寬度的多行，優先在底線 `_` 處斷行（識別字命名習慣），
 * 單一片段仍超長才強制切字。跟 wrapComment 一樣用「字數」而不是像素，
 * 確保 layout 算出來的高度與實際畫出來的一致。
 */
export function wrapCompactName(
  name: string,
  charsPerLine: number = CARD_METRICS.compactNameWrapChars,
  maxLines: number = CARD_METRICS.compactNameMaxLines,
): string[] {
  if (textWidth(name) <= charsPerLine) return [name];

  const segments = name.split(/(?<=_)/);
  const lines: string[] = [];
  let current = "";
  for (const segment of segments) {
    if (current && textWidth(current + segment) > charsPerLine) {
      lines.push(current);
      current = segment;
    } else {
      current += segment;
  ...
  for (const line of lines) {
    if (textWidth(line) <= charsPerLine) {
      wrapped.push(line);
      continue;
    }
    for (let i = 0; i < line.length; i += charsPerLine) wrapped.push(line.slice(i, i + charsPerLine));
  }
...
export function buildCardModel(table: Table, state: ViewState): CardModel {
  ...
  // 精簡模式的卡片幾何跟其他層級完全不同（固定寬度、表名換行），
  // 獨立算完直接回傳，不跟下面共用的欄位/寬度估算邏輯混在一起。
  if (detailLevel === "compact") {
    const compactNameLines = wrapCompactName(table.name);
    return {
      table,
      detailLevel,
      collapsed: false,
      ...
      group: table.group,
      width: CARD_METRICS.compactCardWidth,
      height:
        CARD_METRICS.compactPaddingY * 2 +
        compactNameLines.length * CARD_METRICS.compactNameLineHeight +
        CARD_METRICS.compactMetaHeight,
      compactNameLines,
    };
  }
