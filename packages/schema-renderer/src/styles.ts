...
/* move 模式（schemalens-web 新增）：拖曳空白背景才會平移，游標比照傳統拖曳畫布工具。
   select 模式（預設）維持十字游標，提示「拖曳空白背景 = 框選」。 */
.dbs-root.is-move-mode { cursor: grab; }
.dbs-root.is-move-mode.is-panning { cursor: grabbing; }
...
/* centerOn / fitView 用的短暫過場（schemalens-web 新增）；拖曳平移與滾輪縮放
   不會加這個 class，維持即時跟手，不會有動畫延遲。 */
.dbs-viewport.dbs-animate-transform {
...
/* "compact" detail level（schemalens-web 精簡模式專用）：固定寬度的極簡卡片，
   只有表名（可換行）跟 schema/欄位數一行，不畫欄位、群組色條、collapse 按鈕。 */
.dbs-card.is-compact {
  display: flex;
...
.dbs-card-compact-name {
  display: flex;
  flex-direction: column;
  font-weight: 700;
  line-height: 17px;
}
.dbs-card-compact-name-line {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.dbs-card-compact-meta {
  font-size: 10px;
  color: var(--vscode-descriptionForeground, #9d9d9d);
}
.dbs-card-group {
