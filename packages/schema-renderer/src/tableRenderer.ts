...
export function renderCard(
  ...
  if (card.group) root.dataset.group = card.group;
  if (card.group && card.detailLevel !== "compact") {
    // 左緣色條：掃視 150 張表時，顏色比文字更快辨識模組。
    // compact 卡片跳過這個——外層已經有整片分區色塊在標示群組，
    // 每張卡片再畫一次色條只是雜訊。
  ...
  if (card.detailLevel === "compact") {
    root.classList.add("is-compact");
    const name = el("div", "dbs-card-compact-name");
    for (const line of card.compactNameLines ?? [card.table.name]) {
      name.append(el("span", "dbs-card-compact-name-line", line));
    }
    const meta = el(
      "div",
      "dbs-card-compact-meta",
      `${card.table.schema} · ${card.table.columns.length} 欄`,
    );
    root.append(name, meta);
    return { root, rowByColumn: new Map() };
  }
