 ...
 *
 * "compact"（schemalens-web 新增，VS Code 插件不會用到）：專供精簡模式用的
 * 極簡卡片——固定寬度、表名換行、只有一行 schema/欄位數，連 Overview 保留的
 * 群組徽章／備註／collapse 按鈕都不畫，因為那些資訊已經由外層的分區色塊承擔。
 * 獨立於 "overview" 之外，這樣切換完整模式的「總覽」按鈕行為不受影響。
 */
export type DetailLevel = "overview" | "keys" | "full" | "compact";
