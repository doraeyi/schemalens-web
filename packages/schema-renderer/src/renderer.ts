...
export interface RendererEvents {
  ...
  /** 縮放或平移後觸發（zoomBy / fitView / resetView / centerOn / 拖曳 / 滾輪）。 */
  viewportChanged(scale: number): void;
}

/**
 * 背景互動模式（schemalens-web 新增，VS Code 插件的被動 Preview 用不到）：
 *   "select" — 預設。拖曳空白背景不做任何事，避免誤觸平移。
 *   "move"   — 拖曳空白背景才會平移畫布。
 * 拖曳卡片本身、滾輪縮放/平移不受此影響。
 */
...
export class SchemaRenderer {
  ...
  private ty = 0;
  private scale = 1;
  private animateTimeout: ReturnType<typeof setTimeout> | undefined;
  /**
   * schemalens-web 新增：精簡模式的分區版面本來就是由上往下排好的固定順序，
   * 點表格/關聯再自動把畫面 pan 過去反而讓人覺得「跳來跳去」，所以精簡模式
   * 會把這個關掉，只更新 focus 狀態（卡片 emphasis 仍然正常），不移動視口。
   */
  private autoCenterOnFocus = true;
   ...
   * 更新檢視狀態。
   *
   * 只有會改變卡片幾何的欄位（detailLevel / layoutMode / collapsed）需要重排版；
  ...
  setAutoCenterOnFocus(enabled: boolean): void {
    this.autoCenterOnFocus = enabled;
  }
  ...
  /** 切換背景拖曳行為：select（不做事）或 move（平移）。 */
  setInteractionMode(mode: InteractionMode): void {
    this.interactionMode = mode;
    this.root.classList.toggle("is-move-mode", mode === "move");
  }
  ...
  /**
   * 換一顆 LayoutEngine（schemalens-web 新增：精簡模式用分區分帶版面，
   * 跟完整模式的 layered layout 是不同演算法）。手動拖曳過的位置在舊版面下
   * 才有意義，換算法後直接丟棄，立即用新版面重新排一次並回到 Fit View。
   */
   ...
   *
   * `layoutMode !== "group"` 時，Layout 不會聚攏同群組的表（純依關聯排版），
   * `groupBounds` 自然是空的，這裡不需要另外判斷就會什麼都不畫。
   */
  ...
  private applyEmphasis(): void {
    ...
    // （含欄位聚焦時把 related/filtered/highlight 都算進去），這裡不重複判斷。
    ...
    for (const [tableId, elements] of this.cardElements) {
      ...
      // 淡化／隱藏是一個軸（透明度與顯示），邊框強調是另一個軸，兩者都直接來自
      // resolveVisibility，這裡不再做任何額外判斷。
  ...
  /** 把某張卡片移到新的圖座標，並即時更新相鄰的關聯線與所屬群組外框。 */
  ...
  private applyTransform(): void {
    ...
    this.events.viewportChanged?.(this.scale);
  }

  /**
   * 跟 applyTransform 一樣，但補一段短暫的 CSS transition（schemalens-web
   * 新增：使用者反映每次點表格都「瞬間跳過去」很生硬）。
   * 只用在 centerOn / fitView 這種「跳到某個地方」的操作；拖曳平移、滾輪縮放
   * 這種需要跟手、逐格更新的互動維持原本的 applyTransform，不然會延遲卡手。
   */
  private animateTransform(): void {
    this.viewport.classList.add("dbs-animate-transform");
    this.applyTransform();
    clearTimeout(this.animateTimeout);
    this.animateTimeout = setTimeout(() => {
      this.viewport.classList.remove("dbs-animate-transform");
    }, 260);
  }
  ...
  private bindPointerEvents(): void {
    ...
    this.root.addEventListener("pointerdown", (event) => {
      ...
      // 但**不能就此中止**——聚焦時淡化的卡片佔了大半畫面，中止的話那一大片
      // 就變成連平移都做不了的死區，所以往下走，跟一般空白背景一樣處理。
      const draggableCard =
        card && !card.classList.contains("is-dimmed") && !card.classList.contains("is-filtered-out")
          ? card
          : null;

      if (card) {
        // 摺疊鈕與 "+N more" 有自己的行為，不當成拖曳或平移的起點。
      ...
      // select 模式下拖曳空白背景不做任何事（schemalens-web 需求：
      // 避免跟框選手勢衝突），只有 move 模式才平移。淡化卡片落到這裡時也一樣。
    ...
    const endPointer = (event: PointerEvent): void => {
      ...
      if (panMoved) {
        // 平移過就吃掉隨後的 click，否則從淡化卡片起手平移，放開手會誤把焦點跳過去。
        suppressClick = true;
      }
      panMoved = false;
      this.root.releasePointerCapture?.(event.pointerId);
    ...
    this.root.addEventListener("click", (event) => {
      ...
      if (column) {
        ...
        if (same) {
          ...
          // 畫面上兩個焦點各說各話。
          // 不直接呼叫 focusTable() / focusColumn()，因為它們可能觸發置中——
